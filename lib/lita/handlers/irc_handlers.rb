module ForecastIo
  module IrcHandlers
    ...
    def handle_irc_ansitest(response)
      ...
      response.reply "\x03#{c[:royal]}█\x03#{c[:blue]}█\x03#{c[:teal]}█\x03#{c[:aqua]}█\x03#{c[:purple]}█\x03#{c[:green]}█\x03#{c[:lime]}█\x03#{c[:yellow]}█\x03#{c[:orange]}█\x03#{c[:red]}█\x03#{c[:pink]}█\x03#{c[:brown]}█"
    end
  end
end
